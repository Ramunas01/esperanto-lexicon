...
* \[x] src/analyzer/coverage\_report.py — `load\_inflected\_forms()` added; `classify\_tokens()` now checks inflected\_forms as steps 3-4 before falling back to UNKNOWN. Common English function words ('is', 'are', 'was', 'were', 'has', 'had') now correctly classified as TIER1 via inflected\_forms → 'be'/'have' → concept\_lang. Proficiency baseline v2 run: control T4\_ratio 0.009, novice 0.053, expert 0.066. 39 tests passing.
