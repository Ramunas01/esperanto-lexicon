...
* \[x] src/extractor/statistical\_mwe\_detector.py — rewritten: DOC\_DELIMITER='♥' document boundaries; Token NamedTuple(text,lemma,pos); POS-shape filter (noun-phrase patterns only); lemma-based lexicon filter; doc-count-aware ranking (doc\_count DESC, assoc\_score DESC, freq DESC); --min-doc-count (default 3); NE bucket removed; output adds pos\_pattern/doc\_count/association\_score/sample\_context fields. Real corpus: 3268 raw → 411 after all filters. 83 tests passing.
...
* \[x] src/analyzer/conflict\_report.py — cross-domain `--lang` now defaults to `auto` (detects common languages between the two DBs); genuine definitional conflicts distinguished from coverage gaps (one side has no definition) via `CrossConflict.incomplete` flag and separate count in the report. 42 tests passing.
